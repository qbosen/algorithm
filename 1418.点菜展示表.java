...
class Solution {
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        orders.forEach(list -> {
            ...
            map
                    .compute(Integer.parseInt(list.get(1)), (k, v) -> v == null ? new HashMap<>() : v)
                    .compute(list.get(2), (k, v) -> v == null ? 1 : v + 1);
        });
        ...
        for (Integer table : tables) {
            ...
            Map<String, Integer> counter = map.get(table);
            for (String food : foods) {
                row.add(String.valueOf(counter.getOrDefault(food, 0)));
            }
            res.add(row);
        }
        return res;
